...
h36m_edges = [[0, 1], [1, 2], [2, 3], [0, 4], [4, 5], [5, 6], [0, 7],
                    [7, 8], [8, 9], [9, 10], [8, 11], [11, 12], [12, 13],
                    [8, 14], [14, 15], [15, 16],[8, 10] ]


class Debugger(object):
  def __init__(self, ipynb=False, edges=h36m_edges):
    self.ipynb = ipynb
    if not self.ipynb:
      self.plt = plt
  ...
  def add_point_3d(self, points, c='b', marker='o', edges=None):
    ...
    print("Shape of total points are",points.shape)
    x, y, z = np.zeros((3, points.shape[0]))# XYZ ARE THE ORIGINAL POINTS
    ...
    self.ax.scatter(x, y, z, s = 20, c = c, marker = marker)
    for e in edges:
